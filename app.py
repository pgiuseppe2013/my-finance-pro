...
# 4. FUNZIONE WIDGET UNIFICATO PER INSERIMENTO MOVIMENTO (Semplificato e Intuitivo)
def render_movement_form(key_suffix=""):
    if not st.session_state.accounts:
        st.info("⚠️ Crea prima almeno un Conto o una Carta di Credito dalla Dashboard per poter inserire movimenti.")
        return

    with st.container(border=True):
        st.markdown("### ➕ Inserisci Nuovo Movimento")
        
        # Sincronizzazione automatica Data Contabile -> Valuta
        if f'dc_{key_suffix}' not in st.session_state:
            st.session_state[f'dc_{key_suffix}'] = date.today()
        if f'dv_{key_suffix}' not in st.session_state:
            st.session_state[f'dv_{key_suffix}'] = date.today()

        col1, col2, col3 = st.columns(3)
        with col1:
            dc = st.date_input("Data Contabile", key=f"dc_input_{key_suffix}")
        with col2:
            if dc != st.session_state[f'dc_{key_suffix}']:
                st.session_state[f'dv_{key_suffix}'] = dc
                st.session_state[f'dc_{key_suffix}'] = dc
            dv = st.date_input("Data Valuta", value=st.session_state[f'dv_{key_suffix}'], key=f"dv_input_{key_suffix}")
            if dv != st.session_state[f'dv_{key_suffix}']:
                st.session_state[f'dv_{key_suffix}'] = dv
        with col3:
...
# 5. BARRA DI NAVIGAZIONE A PULSANTI PRONTI
...
if menu == "DASHBOARD":
    ...
    # Inserimento Semplificato Unico per Conto / Carta (Disponibile SOLO nella Dashboard)
    with st.container(border=True):
...
# --- REPORT ---
    ...
    # Inserimento movimento rapido disponibile anche qui nel Report
    with st.expander("➕ Inserisci rapido movimento da Report"):
...
# --- IMPOSTAZIONI ---
    ...
    # Inserimento movimento rapido disponibile anche qui nelle Impostazioni
    with st.expander("➕ Inserisci rapido movimento da Impostazioni"):
