    ...
    with tab_register:
        ...
        if st.button("Registrati", key="btn_register_action"):
            if not r_user.strip() or not r_pass.strip():
                ...
                st.success("Account creato con successo! Ora puoi effettuare il login nella scheda 'Accedi'.")
    st.stop()
...
# Filtro dei dati specifici dell'utente loggato
def get_user_accounts():
    return [a for a in st.session_state.accounts if a.get('user') == st.session_state.logged_user]

def get_user_movements():
    return [m for m in st.session_state.movements if m.get('user') == st.session_state.logged_user]
...
def get_account_balance(acc):
    ...
    user_movs = get_user_movements()
    valid_movements = [
        m for m in user_movs 
...
def add_movement(m_date_c, m_date_v, acc_id, m_type, cat, desc, amt, repeat=False, freq="Mensile", count=1):
    ...
    acc = next((a for a in user_accounts if a['id'] == acc_id), None)
...
def render_movement_form(key_suffix=""):
    ...
    with st.expander(t("add_mov")):
        ...
        with col3:
            acc_choice = st.selectbox(t("sel_acc"), [a['name'] for a in user_accounts], key=f"acc_sel_{key_suffix}")
        ...
        if st.button(t("btn_reg_mov"), key=f"btn_save_mov_{key_suffix}"):
            a_id = next(a['id'] for a in user_accounts if a['name'] == acc_choice)
...
# 7. BARRA DI NAVIGAZIONE A PULSANTI & INFO UTENTE
st.title(t("title"))
...
with col_nav2:
    st.markdown(f"👤 **{st.session_state.logged_user}**")
...
user_accounts = get_user_accounts()
user_movements = get_user_movements()

# --- DASHBOARD ---
if menu == "DASHBOARD":
    st.subheader(t("panoramica"))
    
    total_cash = sum(get_account_balance(a) for a in user_accounts if a['type'] != "Carta di Credito")
        ...
        with cols[i % 3]:
            with st.container(border=True):
                ...
                if acc['type'] == "Carta di Credito":
                    used = abs(sum(m['amt'] for m in user_movements if m['acc_id'] == acc['id'] and m['amt'] < 0 and not m.get('virtual')))
...
# --- MOVIMENTI ---
    ...
    real_movs = [m for m in user_movements if not m.get('virtual')]
    if search:
